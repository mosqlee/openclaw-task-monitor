...
const PLAN_FILE = path.join(PLAN_DIR, "execution-plan.md"); // 固定文件名，MVP 单任务
const planCache = { content: null, mtimeMs: 0 }; // 单文件缓存
...
// 异步读取 plan（带 mtime 缓存）
async function readPlan() {
  try {
    const stats = await fs.promises.stat(PLAN_FILE);
    // 缓存命中且文件未修改
    if (planCache.content && planCache.mtimeMs === stats.mtimeMs) {
      return planCache.content;
    }
    const content = await fs.promises.readFile(PLAN_FILE, "utf-8");
    planCache.content = content;
    planCache.mtimeMs = stats.mtimeMs;
    return content;
  } catch (err) {
...
async function cleanupExpiredPlans() {
  try {
    // 固定文件名模式，只检查 execution-plan.md 的修改时间
    const stats = await fs.promises.stat(PLAN_FILE).catch(() => null);
    if (stats) {
      const maxAge = 24 * 60 * 60 * 1000; // 24h
      if (Date.now() - stats.mtimeMs > maxAge) {
        await fs.promises.unlink(PLAN_FILE);
        planCache.content = null;
        debugLog(`PLAN_EXPIRED_CLEANUP: execution-plan.md`);
      }
    }
  } catch (e) {
...
module.exports = {
  register(api) {
    ...
    api.on("agent_end", (event, ctx) => {
      ...
      fs.promises.readFile(PLAN_FILE, "utf-8").then(content => {
    ...
    api.on("before_prompt_build", async (event, ctx) => {
      ...
      if (status === "DONE") {
        ...
        try {
          await fs.promises.unlink(PLAN_FILE);
          planCache.content = null;
          debugLog(`PLAN_CLEANED_DONE`);
        } catch {}
        return;
      }

      // 注入未完成的执行计划
      debugLog(`PLAN_INJECTED`);
      return {
